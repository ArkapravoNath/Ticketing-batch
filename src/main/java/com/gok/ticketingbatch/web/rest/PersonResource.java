...
/**
 * REST controller for managing {@link com.gok.ticketingbatch.domain.Person}.
 */
@RestController
@RequestMapping("/api")
public class PersonResource {
    ...
    private static final String ENTITY_NAME = "ticketingBatchPerson";
    ...
    public ResponseEntity<List<PersonDTO>> getAllPeople(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("medicalpractitioner-is-null".equals(filter)) {
            log.debug("REST request to get all Persons where medicalPractitioner is null");
            return new ResponseEntity<>(personService.findAllWhereMedicalPractitionerIsNull(),
                    HttpStatus.OK);
        }
